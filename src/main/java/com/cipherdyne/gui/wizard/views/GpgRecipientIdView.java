 ...
 */
public class GpgRecipientIdView extends AbstractView {

    @Override
    public void initialize(Map<EnumWizardVariable, IFwknopVariable> varMap, Map<EnumWizardButton, JButton> btnMap) {
        varMap.put(EnumWizardVariable.GPG_RECIPIENT_ID, new JFwknopTextField(""));
        btnMap.put(EnumWizardButton.BROWSE_FOR_GPG_RECIPIENT_ID, new JButton(EnumWizardButton.BROWSE_FOR_GPG_RECIPIENT_ID.getDescription()));

        // Add object to the newly created panel
        this.add(Utils.createItem(varMap, EnumWizardVariable.GPG_RECIPIENT_ID), "growx");
        this.add(btnMap.get(EnumWizardButton.BROWSE_FOR_GPG_RECIPIENT_ID), "growx");
    }

    @Override
    public EnumWizardView getNextPanel() {
        return EnumWizardView.SETUP_HMAC;
    }
}
